...
class Computer:
    ...
    ) -> tuple[int, dict[str, ndarray[Any, dtype[floating[_64Bit]]]]]:
        """
    ...
    def _ssa(
        ...
        2.2. We want to return the reconstructed time series. In this case, we return
             the reconstructed time series, which we need the full SVD for
        ...
        """
        if config.svd_method == "randomised":
            """
            ...
            U, S, V = self._randomised_svd(trajectory_matrix, rank)
            truncated_svd = U @ np.diag(S) @ V
            self.logger.info("Returning reconstructed time series")
            series = self._get_series_from_truncated_svd(truncated_svd)
            ...
            if config.return_data == "reconstruction":
                ...
                time_series_components = self._parse_components(
    ...
    def _get_series_from_truncated_svd(
        self, truncated_svd: npt.NDArray[np.float64]
    ...
    def _parse_components(
        ...
        for i, singular_value in enumerate(S):
            reconstruction = self._get_elementary_matrix(singular_value, U, V, i)
            components[i] = self._get_series_from_truncated_svd(reconstruction)
        return components

    def _get_elementary_matrix(
        self,
        sing_val: Number,
        U: npt.NDArray[np.float64],
        V: npt.NDArray[np.float64],
